...
router.patch("/:id", async (req, res) => {
    try {
        ...
        const result = await gameData.updateGame(
            gameId,
            userId,
            desiredParticipants,
            skillLevel
...
router.get("/locations/:id", async (req, res) => {
    try {
        ...
        let location = await locationData.getLocationById(id);
        ...
        for (let i = 0; i < gameList.length; ++i) {
            ...
            gameList[i].date = gameList[i].startTime.toLocaleDateString(
                "en-US",
                {
                    year: "numeric",
                    month: "long",
            ...
            gameList[i].endTimeFmt = gameList[i].endTime.toLocaleTimeString(
                "en-US",
                {
                    hour: "2-digit",
                    minute: "2-digit",
        ...
        /* 
         * Ensure facilities fields are not null for rendering. 
         */
        if (location.facilities.basketball === null) {
            location.facilities.basketball = { numCourts: 0 };
        }
        if (location.facilities.tennis === null) {
            location.facilities.tennis = { numCourts: 0 };
        }
...
router.post("/create", async (req, res) => {
    try {
        const locationId = validation.checkId(
            req.body.locationId,
            "Location ID"
        );
        const userId = validation.checkId(req.session.user.userId, "User ID");
        ...
        const endTime = validation.checkISO8601String(
            req.body.endTime,
            "End Time"
        );
